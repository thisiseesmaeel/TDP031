...
class TestNISClient:
    ...
    def test_nsswitch_conf_passwd(self):
        nsswitch_config = subprocess.run(['cat /etc/nsswitch.conf'], stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8')
        assert re.search(r'\npasswd:.* nis', nsswitch_config) != None

    def test_nsswitch_conf_group(self):
        nsswitch_config = subprocess.run(['cat /etc/nsswitch.conf'], stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8')
        assert re.search(r'\ngroup:.* nis', nsswitch_config) != None
    
    def test_nsswitch_conf_shadow(self):
        nsswitch_config = subprocess.run(['cat /etc/nsswitch.conf'], stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8')
        assert re.search(r'\nshadow:.* nis', nsswitch_config) != None
